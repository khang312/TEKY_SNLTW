...
def new_window():
    ...
    btn_new = tkinter.Button(window, command= login, text = 'register')
    btn2_new = tkinter.Button(window, command= quit, text = 'Quit')

    name_new = tkinter.Entry(window)
    date_new = tkinter.Entry(window)

    quoc_tich = ttk.Combobox(window)
    quoc_tich['value'] = ('Vietnam',  'China',  'Thailand')
    name1_new = tkinter.Label(window, text= 'Ho va Ten')
    date1_new = tkinter.Label(window, text= 'Ngay Sinh')
    quoc_tich1_new= tkinter.Label(window, text= 'Quoc Tich')
    quoc_tich.current(0)


    gender_var = tkinter.StringVar(value = 'none')
    radio_female_new =tkinter.Radiobutton(window, text = 'nữ', variable= gender_var, value= 'female')
    radio_male_new =tkinter.Radiobutton(window, text = 'nam', variable= gender_var, value= 'male')
